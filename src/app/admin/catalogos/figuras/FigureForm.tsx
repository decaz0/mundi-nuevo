...
import { createFigure } from "@/actions/catalogActions";
...
export default function FigureForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name) return alert("Escribe el nombre de la figura");
    setLoading(true);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <input value={name} onChange={e => setName(e.target.value)} required className="w-full border rounded-lg p-2 text-sm outline-none focus:ring-2 focus:ring-premia-red" placeholder="Ej. Silueta Deportiva" />
      </div>

      <ImageUploader
        folder="catalogos/figuras"
        onUploaded={url => setImageUrl(url)}
        label="Imagen de la figura"
        size="lg"
        placeholder="Subir imagen de la figura"
      />
